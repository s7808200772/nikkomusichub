...
> 本文件整合安裝、驗收、操作、維修、資安、資料保留與長期維運說明。已根據目前 `security-final` 程式碼更新，移除了過時的 QNAP 專用描述、`/library` 獨立頁面、`device_id`/`role` 欄位等不再準確的內容。
...
- [ ] 喇叭或擴大機已連接並測試音量。
- [ ] 已加入 Tailscale 網路，並可 ping 到 WebDAV 主機（預設為 QNAP Tailscale IP）。
- [ ] WebDAV 已啟用，且遠端音樂資料夾（預設 `/NikkoMusic`）存在。
...
1. 將 MP3 放入 WebDAV 遠端音樂資料夾（預設 `/NikkoMusic`）。
2. Cloud `/stores` 選取店點，點「同步選取店點」。
...
- 手動：目前可呼叫 `/api/backup/create`；Pi UI 尚未提供按鈕。
...
| 症狀 | 可能原因 | 排查步驟 |
|---|---|---|
| 播放器無聲 | 音訊輸出裝置錯誤 / 喇叭未開 | Pi Settings 測試音訊輸出，確認裝置與音量 |
| Cloud 顯示離線 | MQTT agent 未啟 / Tailscale 斷線 | `systemctl status nikko-music-mqtt.service`、`tailscale status` |
| 同步失敗 | WebDAV 設定錯 / 遠端主機離線 | `/api/health/qnap`、檢查 rclone.conf |
| 磁碟空間不足 | log 或備份累積 | 清理 `logs/` 與 `backups/`、檢查磁碟使用率 |
| CPU 溫度過高 | 散熱不良 | 檢查風扇、外殼通風、考慮降頻 |
| OTA 後無法連線 | 更新衝突或依賴失敗 | SSH 登入執行 rollback：`sudo bash /srv/nikko-music/scripts/restore.sh <backup>` 或 `git checkout rollback-before-ota` |
...
2. 重新執行 `install.sh`。
3. 從最新 backup tar.gz 還原：`sudo bash /srv/nikko-music/scripts/restore.sh <backup>`。
4. 確認 Store ID、MQTT 設定正確，啟動服務。

### 6.2 NAS / 遠端主機失聯

- Pi 會繼續播放本地 `music/` 的音樂，不受影響。
- 檢查 Tailscale 與遠端主機網路；修復後下次同步會自動恢復。

### 6.3 Pi 硬體故障

1. 準備備用 Pi，安裝 OS 與專案。
2. 從備份還原設定。
3. 在 Cloud `/stores` 更新設備資訊（若使用不同硬體）。
4. 測試播放與同步。
...
- 持續追蹤 `PROJECT_STATUS_120.md` 與本手冊進度。
